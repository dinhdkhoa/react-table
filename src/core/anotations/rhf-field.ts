...
> = TControlType & {
  ...
  validate?: Record<string, (fieldValue: any, formValue: TEntity) => ValidateResult | Promise<ValidateResult>>
}

export type RHFOptions<
  TEntity extends FieldValues = FieldValues,
  TControlType extends ControlType = ControlType
> = RHFSharedType<TEntity, TControlType>

// Decorator factory for react-hook-form
export function RHF<TEntity extends FieldValues>(options: RHFOptions<TEntity>) {
  return function (target: any, propertyKey: keyof TEntity & string) {
    const fields = Reflect.getMetadata(RHF_FIELDS, target) || {}
    if (!options.placeholder) options.placeholder = options.label
